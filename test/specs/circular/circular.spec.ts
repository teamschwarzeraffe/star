...
describe("Schema with circular (recursive) $refs", () => {
  describe("$ref to self", () => {
    it("should parse successfully", async () => {
      ...
      let schema = await parser.parse(path.rel("test/specs/circular/circular-self.yaml"));
    ...
    it("should dereference successfully", async () => {
      let parser = new $RefParser();
      let schema = await parser.dereference(path.rel("test/specs/circular/circular-self.yaml"));
      expect(schema).to.equal(parser.schema);
    ...
    it("should double dereference successfully", async () => {
      let firstPassSchema = await $RefParser.dereference(path.rel("test/specs/circular/circular-self.yaml"));
      let parser = new $RefParser();
      let schema = await parser.dereference(firstPassSchema);
      expect(schema).to.equal(parser.schema);
      expect(schema).to.deep.equal(dereferencedSchema.self);
      // The "circular" flag should be set
      expect(parser.$refs.circular).to.equal(true);
    ...
    it("should bundle successfully", async () => {
      let parser = new $RefParser();
      let schema = await parser.bundle(path.rel("test/specs/circular/circular-self.yaml"));
      expect(schema).to.equal(parser.schema);
      expect(schema).to.deep.equal(parsedSchema.self);
      // The "circular" flag should NOT be set
      // (it only gets set by `dereference`)
  ...
  describe("$ref to ancestor", () => {
    it("should parse successfully", async () => {
      let parser = new $RefParser();
      let schema = await parser.parse(path.rel("test/specs/circular/circular-ancestor.yaml"));
      expect(schema).to.equal(parser.schema);
    ...
    it("should dereference successfully", async () => {
      let parser = new $RefParser();
      let schema = await parser.dereference(path.rel("test/specs/circular/circular-ancestor.yaml"));
      expect(schema).to.equal(parser.schema);
    ...
    it("should double dereference successfully", async () => {
      let parser = new $RefParser();
      let firstPassSchema = await $RefParser.dereference(path.rel("test/specs/circular/circular-ancestor.yaml"));
      let schema = await parser.dereference(firstPassSchema);
      expect(schema).to.equal(parser.schema);
      expect(schema).to.deep.equal(dereferencedSchema.ancestor.fullyDereferenced);
      // The "circular" flag should be set
    ...
    it("should bundle successfully", async () => {
      let parser = new $RefParser();
      let schema = await parser.bundle(path.rel("test/specs/circular/circular-ancestor.yaml"));
      expect(schema).to.equal(parser.schema);
      expect(schema).to.deep.equal(parsedSchema.ancestor);
      // The "circular" flag should NOT be set
      // (it only gets set by `dereference`)
  ...
  describe("indirect circular $refs", () => {
    it("should parse successfully", async () => {
      let parser = new $RefParser();
      let schema = await parser.parse(path.rel("test/specs/circular/circular-indirect.yaml"));
      expect(schema).to.equal(parser.schema);
    ...
    it("should dereference successfully", async () => {
      let parser = new $RefParser();
      let schema = await parser.dereference(path.rel("test/specs/circular/circular-indirect.yaml"));
      expect(schema).to.equal(parser.schema);
    ...
    it("should double dereference successfully", async () => {
      let parser = new $RefParser();
      let firstPassSchema = await $RefParser.dereference(path.rel("test/specs/circular/circular-indirect.yaml"));
      let schema = await parser.dereference(firstPassSchema);
      expect(schema).to.equal(parser.schema);
      expect(schema).to.deep.equal(dereferencedSchema.indirect.fullyDereferenced);
      // The "circular" flag should be set
    ...
    it("should bundle successfully", async () => {
      let parser = new $RefParser();
      let schema = await parser.bundle(path.rel("test/specs/circular/circular-indirect.yaml"));
      expect(schema).to.equal(parser.schema);
      expect(schema).to.deep.equal(parsedSchema.indirect);
      // The "circular" flag should NOT be set
      // (it only gets set by `dereference`)
  ...
  describe("indirect circular and ancestor $refs", () => {
    it("should parse successfully", async () => {
      let parser = new $RefParser();
      let schema = await parser.parse(path.rel("test/specs/circular/circular-indirect-ancestor.yaml"));
      expect(schema).to.equal(parser.schema);
    ...
    it("should dereference successfully", async () => {
      let parser = new $RefParser();
      let schema = await parser.dereference(path.rel("test/specs/circular/circular-indirect-ancestor.yaml"));
      expect(schema).to.equal(parser.schema);
    ...
    it("should double dereference successfully", async () => {
      let parser = new $RefParser();
      let firstPassSchema = await parser.dereference(path.rel("test/specs/circular/circular-indirect-ancestor.yaml"));
      let schema = await parser.dereference(firstPassSchema);
      expect(schema).to.equal(parser.schema);
      expect(schema).to.deep.equal(dereferencedSchema.indirectAncestor.fullyDereferenced);
      // The "circular" flag should be set
    ...
    it("should bundle successfully", async () => {
      let parser = new $RefParser();
      let schema = await parser.bundle(path.rel("test/specs/circular/circular-indirect-ancestor.yaml"));
      expect(schema).to.equal(parser.schema);
      expect(schema).to.deep.equal(parsedSchema.indirectAncestor);
      // The "circular" flag should NOT be set
      // (it only gets set by `dereference`)
