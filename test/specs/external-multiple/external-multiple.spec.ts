...
describe("Schema with multiple external $refs to different parts of a file", () => {
  it("should parse successfully", async () => {
    ...
    let schema = await parser.parse(path.abs("test/specs/external-multiple/external-multiple.yaml"));
  ...
  it("should dereference successfully", async () => {
    let parser = new $RefParser();
    let schema = await parser.dereference(path.rel("test/specs/external-multiple/external-multiple.yaml"));
    expect(schema).to.equal(parser.schema);
  ...
  it("should bundle successfully", async () => {
    let parser = new $RefParser();
    let schema = await parser.bundle(path.rel("test/specs/external-multiple/external-multiple.yaml"));
    expect(schema).to.equal(parser.schema);
